...
```
`AutoGude`クラスは引数の`model`内で指定された名前付きの確率変数（ただし`obs`指定されたものを除く）がベイズ学習対象と自動認識し、各変数に対して規定された変分関数を自動生成するものになります。ここでは`AutoGuide`のサブクラスである`AutoDiagonalNormal`を用いており、これはすなわち`model`内の`w`の変分関数を分散共分散行列の対角成分がゼロの多次元正規関数の形をとる変分関数を規定した事に相当します。
